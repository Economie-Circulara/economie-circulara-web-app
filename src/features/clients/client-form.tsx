...
export interface ClientFormProps {
   ...
   * sa explice, la creare, ce se intampla cu emailul: pentru admin clientul primeste
   * automat invitatia in portal (vezi `createClientAction`); invitarea necesita
   * admin (vezi `sendClientInvite`), la fel ca ecranul /setari/utilizatori.
   */
...
export function ClientForm({
  ...
  // Doar la creare: emailul declanseaza automat invitatia in portal (admin).
  const inviteHint =
    mode !== "create"
      ? null
      : currentUserRole === "admin"
        ? "Dacă completezi emailul, clientul primește automat o invitație în portal pentru a-și seta parola."
        : "Invitația în portal o trimite un administrator, din pagina clientului.";
  ...
  return (
    <form action={formAction} className="max-w-2xl space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField label="Email" hint={inviteHint ?? undefined}>
              {(id) => (
