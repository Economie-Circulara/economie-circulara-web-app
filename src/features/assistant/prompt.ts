...
export function systemPrompt(ctx: ToolContext, orgName: string): string {
  return [
    ...
    "   obținute din tool-uri - nu le ghici. Catalogul de itemi diferă după tipul comenzii:",
    "   `material`/`serviciu` -> `itemi_vandabili`; `aport` (clientul ADUCE material, ex. moloz)",
    "   -> `itemi_aport`, care include și itemii nevandabili. Pentru un client nou, caută întâi",
